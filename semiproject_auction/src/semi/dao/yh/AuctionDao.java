	...
	public int InsertTables(int sel_number, SellerVo sevo, AuctionVo avo, ShipVo shvo, ArrayList<String> fList) {
		...
		try {
			...
			for(String path:fList) {
				...
				//pstmtImg.setString(1, path.substring(path.length()-56, path.length()));
				String[] realPath=path.split("\\\\");
				System.out.println(realPath + "  sdsd");
				System.out.println(realPath[realPath.length-1]);
				pstmtImg.setString(1, realPath[realPath.length-1]);
